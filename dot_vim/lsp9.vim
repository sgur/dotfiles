...
" json
let s:catalog_file = expand('~/.cache/schemastore/catalog.json')
let s:schemas = filereadable(s:catalog_file)
      \ ? readfile(s:catalog_file)->join()->json_decode()['schemas']
      \ : []
...
let s:lsp_servers += [#{
      ...
      \ args: ['lsp', 'stdio'],
      \ initializationOptions: s:taplo_lsp_options,
      \ workspaceConfig: #{
      \   evenBetterToml: s:taplo_lsp_options
      \ }
      \}]
...
" yaml
function! s:schemas_map() abort "{{{
  let result = {}
  for v in s:schemas
    if has_key(v, 'fileMatch')
      let result[v['url']] = v['fileMatch']
    endif
  endfor
  return result
endfunction "}}}
