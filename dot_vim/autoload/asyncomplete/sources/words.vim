...
function! asyncomplete#sources#words#get_source_options(opts)
  let l:config = extend({
...
function! asyncomplete#sources#words#completor(opt, ctx)
  let l:col = a:ctx['col']
  let l:typed = a:ctx['typed']

  let l:complete_min_chars = a:opt['config']['complete_min_chars']
  let l:words_file = a:opt['config']['words_file']
...
function! s:exit_handler(info, job, status) abort "{{{
  call asyncomplete#log('asyncomplete-words.vim', 'exitcode', a:status)

  let l:ctx = a:info['ctx']
  let l:start_col = a:info['start_col']

  let l:matches = []
  let l:itembase = {'dup' : 1, 'icase' : 1, 'menu' : '[words]' }
